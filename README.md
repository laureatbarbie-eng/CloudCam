...
Подмодуль определения высоты нижней границы облачности, скорости и направления ветра в слое 300-1500 м — это проект, который позволяет автоматически захватывать фотографии с помощью двух микроконтроллеров ESP32-CAM с камерами OV2640 с линзой 120 и 160 градусов и загружать их на удаленный сервер Raspberry Pi 5. Затем производится расчёт ВНГО по моностатическому методу Зуева, с помощью cv2. Устройство работает автономно, делая снимки по расписанию каждые 10 минут, затем переходит в режим глубокого сна для экономии энергии. После пробуждения устройство подключается к Wi-Fi, делает снимок и отправляет его на сервер.
