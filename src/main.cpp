...
#include <HTTPClient.h> // ДОБАВЛЕНО для OTA
...
static const bool WIFI_MAX_TXPOWER = false;
...
// ДОБАВЛЕНО: Генерация заголовка авторизации
...
// ИСПРАВЛЕНО: переименовано в OV2640 (так как настройки именно для неё)
// ------------------- Camera Init OV2640 -------------------
...
static void checkForOTA() {
  WiFiClient client;
  // Обращаемся к веб-интерфейсу на порт 8080 за файлом прошивки
  String ota_url = String("http://") + SERVER_HOST + ":8080/ota/" + CAM_ID + ".bin";
  Serial.printf("[OTA] Checking for update at %s\n", ota_url.c_str());

  client.setTimeout(15000); // Даем больше времени на скачивание бинарника
  
  // Отключаем проверку версии, просто шьем, если файл доступен
  t_httpUpdate_return ret = httpUpdate.update(client, ota_url);

  switch (ret) {
    case HTTP_UPDATE_FAILED:
      // Ошибка 404 (файла нет) - это нормальное поведение, если мы не загрузили прошивку
      Serial.printf("[OTA] No update found or failed (%d): %s\n", httpUpdate.getLastError(), httpUpdate.getLastErrorString().c_str());
      break;
    ...
    case HTTP_UPDATE_OK:
      ...
      // После успешной прошивки ESP32 перезагрузится автоматически
      break;
  }
}
...
void setup() {
  ...
  startCameraOV2640(); // <-- Вызов исправленной функции
