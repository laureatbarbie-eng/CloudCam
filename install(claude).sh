...
# ─────────────────────────────────────────────────────────────────────────────
step "ФАЗА 2 · ШАГ 1/3 — NetworkManager: изоляция ${AP_IFACE}"
# ─────────────────────────────────────────────────────────────────────────────
# wlan0 остаётся под NM (интернет сохраняется).
# wlan1 добавляется в unmanaged — NM его не трогает.
# nmcli general reload conf — не рвёт текущие соединения.

mkdir -p /etc/NetworkManager/conf.d
cat > /etc/NetworkManager/conf.d/99-cloudcam-ap.conf <<NM
# CloudCam: ${AP_IFACE} (MT7612U) управляется hostapd — NM его не трогает
[keyfile]
unmanaged-devices=interface-name:${AP_IFACE}
NM

if systemctl is-active NetworkManager &>/dev/null; then
  info "NM reload (без разрыва wlan0)..."
  nmcli general reload conf 2>/dev/null || \
    systemctl reload NetworkManager 2>/dev/null || true
  sleep 2
fi
success "NM будет игнорировать ${AP_IFACE}"

# Ждём появления интерфейса
info "Ожидание ${AP_IFACE} (до 20 сек)..."
for i in $(seq 1 20); do
  if ip link show "$AP_IFACE" &>/dev/null; then
    success "Интерфейс ${AP_IFACE} найден"; break
  fi
  [[ $i -eq 20 ]] && \
    warn "${AP_IFACE} не появился — AP активируется после reboot"
  sleep 1
done
rfkill unblock wifi 2>/dev/null || true

# ─────────────────────────────────────────────────────────────────────────────
step "ФАЗА 2 · ШАГ 2/3 — hostapd: точка доступа на ${AP_IFACE}"
# ─────────────────────────────────────────────────────────────────────────────
# hw_mode=g + ieee80211n=1 — 2.4 ГГц с HT.
# ESP32-CAM поддерживает только 2.4 ГГц, 5 ГГц не используем.
...
success "hostapd.conf создан"

# ─────────────────────────────────────────────────────────────────────────────
step "ФАЗА 2 · ШАГ 3/3 — dhcpcd, dnsmasq, NAT, systemd ordering"
# ─────────────────────────────────────────────────────────────────────────────

# Статический IP на wlan1 через dhcpcd
sed -i '/# BEGIN CloudCam AP/,/# END CloudCam AP/d' /etc/dhcpcd.conf 2>/dev/null || true
...
# dnsmasq — резервная копия оригинала
[[ -f /etc/dnsmasq.conf ]] && \
  cp /etc/dnsmasq.conf "/etc/dnsmasq.conf.backup_$(date +%Y%m%d_%H%M%S)" || true
...
log-queries
log-dhcp
log-facility=/var/log/cloudcam_dns.log
DNSMASQ

# IP forwarding
echo "net.ipv4.ip_forward=1" > /etc/sysctl.d/99-cloudcam.conf
sysctl -p /etc/sysctl.d/99-cloudcam.conf -q

# NAT через uplink (eth0 или wlan0)
UPLINK=$(ip route get 8.8.8.8 2>/dev/null \
  | awk '{for(i=1;i<=NF;i++) if($i=="dev") print $(i+1)}' \
  | head -1 || true)
if [[ -n "$UPLINK" && "$UPLINK" != "$AP_IFACE" ]]; then
  info "NAT: ${AP_IFACE} → ${UPLINK}"
  iptables -t nat -C POSTROUTING -o "$UPLINK" -j MASQUERADE 2>/dev/null || \
    iptables -t nat -A POSTROUTING -o "$UPLINK" -j MASQUERADE
  iptables -C FORWARD -i "$UPLINK" -o "$AP_IFACE" \
    -m state --state RELATED,ESTABLISHED -j ACCEPT 2>/dev/null || \
    iptables -A FORWARD -i "$UPLINK" -o "$AP_IFACE" \
    -m state --state RELATED,ESTABLISHED -j ACCEPT
  iptables -C FORWARD -i "$AP_IFACE" -o "$UPLINK" -j ACCEPT 2>/dev/null || \
    iptables -A FORWARD -i "$AP_IFACE" -o "$UPLINK" -j ACCEPT
  netfilter-persistent save || true
  success "NAT настроен через ${UPLINK}"
else
  warn "Uplink не определён — NAT не настроен (не критично для работы системы)"
fi

# Systemd ordering: dhcpcd → hostapd (BindsTo wlan1) → dnsmasq
mkdir -p /etc/systemd/system/hostapd.service.d
cat > /etc/systemd/system/hostapd.service.d/10-cloudcam.conf <<SYSD
[Unit]
...
cat > /etc/systemd/system/dnsmasq.service.d/10-cloudcam.conf <<SYSD
[Unit]
...
success "AP настроена: SSID=${AP_SSID}  IP=${AP_IP}  iface=${AP_IFACE}"
